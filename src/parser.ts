...
export function validateSQL(parsedQuery: SQLQuery) {
    ...
    if (parsedQuery.select) {
        parsedQuery.select.forEach(field => {
            const [table, fieldName] = field.includes('.') ? field.split('.') : [parsedQuery.from, field];
    ...
    parsedQuery.joins.forEach(join => {
        ...
        conditionParts.forEach(part => {
            ...
            if (!isValidField(table, fieldName)) {
                errors.push(`Campo '${fieldName}' não existe na tabela '${table}' na condição JOIN.`);
            }
        });
    });
    ...
    if (parsedQuery.where) {
        ...
        whereConditions.forEach(condition => {
            ...
            if (!isValidField(table, fieldName)) {
                errors.push(`Campo '${fieldName}' não existe na tabela '${table}' na condição WHERE.`);
            }
        });
    }
